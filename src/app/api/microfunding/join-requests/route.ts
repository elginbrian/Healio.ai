...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingMember = await PoolMember.findOne({ pool_id: pool._id, user_id: userId });
    if (existingMember) {
      return NextResponse.json({ success: false, message: "Anda sudah menjadi anggota pool ini." }, { status: 409 });
    }

    const existingRequest = await JoinRequest.findOne({ pool_id: pool._id, user_id: userId, status: JoinRequestStatus.PENDING });
    if (existingRequest) {
    ...
    return NextResponse.json(
      {
        ...
        joinRequest: {
          _id: newJoinRequest._id.toString(),

...
const PageHeader: React.FC<PageHeaderProps> = ({ poolId, userProfileImage }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
      ...
      <NotifProfile profileImageSrc={userProfileImage} />
    </div>
  );
};
...
type PaymentFormProps = {
  ...
  isMidtransAvailable: boolean;
  isChecking: boolean;
  errorMessage: string | null;
  retryLoading: () => void;
};

const PaymentForm: React.FC<PaymentFormProps> = ({ poolDetails, amountToPay, setAmountToPay, handlePayNow, isProcessingPayment, isMidtransAvailable, isChecking, errorMessage, retryLoading }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 md:p-8">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500">Anda dapat memilih metode pembayaran (seperti GoPay, Transfer Bank Virtual Account, Kartu Kredit, dll.) melalui halaman pembayaran Midtrans setelah menekan tombol "Lanjutkan Pembayaran".</p>
      </div>
      ...
      {isChecking ? (
        ...
        !isMidtransAvailable && (
          <div className="text-xs text-red-500 mt-2 text-center">
            ...
            {errorMessage && <p className="text-xs text-gray-500 mt-1">Penyebab: {errorMessage}</p>}
            <button onClick={retryLoading} className="inline-flex items-center text-[var(--color-p-300)] mt-1 hover:underline">
              <RefreshCw className="w-3 h-3 mr-1" /> Muat ulang Midtrans
            </button>
            <button onClick={() => window.location.reload()} className="inline-flex items-center text-[var(--color-p-300)] mt-1 ml-3 hover:underline">
...
const PaymentSummary: React.FC<PaymentSummaryProps> = ({ poolDetails, amountToPay }) => {
  return (
...
type PaymentHistoryProps = {
  ...
  checkPaymentStatus: (contributionId: string) => Promise<any>;
};

const PaymentHistory: React.FC<PaymentHistoryProps> = ({ paymentHistory, isLoadingHistory, checkPaymentStatus }) => {
...
const ErrorState: React.FC<ErrorStateProps> = ({ onBack }) => (
  <div className="p-8 text-center">
    ...
    <button onClick={onBack} className="bg-[var(--color-p-300)] text-white py-2 px-4 rounded-lg hover:bg-[var(--color-p-400)] transition-colors flex items-center mx-auto">
...
const PaymentContent = () => {
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen bg-gray-50">
      <PageHeader poolId={poolId} userProfileImage={(user as any)?.picture || "/img/hospital_dummy.png"} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2">
          <PaymentForm
            ...
            errorMessage={isChecking ? null : "Layanan pembayaran sedang tidak tersedia. Mohon coba lagi nanti."}
            retryLoading={() => {
              console.log("Retry loading Midtrans script");
        ...
        <div className="lg:col-span-1 space-y-6">
          <PaymentSummary poolDetails={poolDetails} amountToPay={amountToPay} />

          <PaymentHistory paymentHistory={paymentHistory} isLoadingHistory={isLoadingHistory} checkPaymentStatus={checkPaymentStatus} />
        </div>
      </div>
    </div>
  );
